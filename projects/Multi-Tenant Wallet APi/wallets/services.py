...
class WalletService:
    ...
    def transfer(tenant_id, sender_wallet_id, recipient_wallet_id, amount, reference_id=None):
        ...
        sender_id_str = str(sender_wallet_id)
        recipient_id_str = str(recipient_wallet_id)

        if sender_id_str == recipient_id_str:
        ...
        with transaction.atomic():
            wallets_dict = Wallet.unscoped_objects.select_for_update().in_bulk(ids)
            
            sender = wallets_dict.get(sender_wallet_id)
            recipient = wallets_dict.get(recipient_wallet_id)
